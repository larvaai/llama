...
def main() -> int:
    ...
    body = {
        ...
        "messages": [
            {"role": "user", "content": "Count the labels that start with A, ignoring case: Alpha, beta, atlas, Gamma."},
        ],
    ...
    semantic_valid = (payload.get("output") or {}).get("result") == 2
    return 0 if status == 200 and payload.get("protocol_valid") and payload.get("output_valid") and semantic_valid else 1
